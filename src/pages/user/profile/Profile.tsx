...
function Profile(props: ProfileProps) {
  ...
  return <PageHeaderWrapper title="个人中心">
    <Modal
      title={<><UserOutlined/> 修改密码</>}
      width={400}
      visible={showModal}
      onOk={() => {
      ...
      <Form form={passwordForm} name="password-form" layout="vertical">
        <CommonForm items={[
          ...
          { label: "重复新密码", name: "reNewPwd", children: <Input.Password/>, required: true },
        ]}/>
      </Form>
    </Modal>
    <Row gutter={20}>
        ...
        <Card>
          <Form form={infoForm} name="profile-form" layout={"vertical"}>
            <div className={classNames("align-center")}>
              <img src={props.login.currentUser?.avatar} alt="头像"
                   style={{ height: 150, width: 150, borderRadius: "50%", backgroundColor: "gray" }}/>
            </div>
            <CommonForm items={formItems} form={infoForm} initialValues={props.login.currentUser}/>
            <Form.Item className="align-right">
              <Button type="dashed" onClick={() => toggleModal(true)}>修改密码</Button>
              <Button htmlType="submit" type="primary" style={{ marginLeft: 15 }}>更新</Button>
            </Form.Item>
          </Form>
        </Card>
      </Col>
